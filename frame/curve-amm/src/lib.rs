		...
		fn exchange(
			...
			let (provider, pool_id, dy, fee) =
				Pools::<T>::try_mutate(pool_id, |pool| -> Result<_, DispatchError> {
					...
					// for (vault_id, assets_id) in
					// 	pool.assets_vault_ids.iter().zip(pool_asset_ids.iter())
					// {
					// let bal = T::LpToken::balance(pool_asset_ids);
					// 	// let res = <T::Vault as StrategicVault>::available_funds(
					// 	// 	vault_id,
					// 	// 	&Self::account_id(&pool_id),
					// 	// );
					// 	// let bal = match res {
			...
			with_transaction(|| {
				let results = Pools::<T>::iter()
					.map(|(pool_id, pool_info)| {
						let pool_account_id = Self::account_id(&pool_id);
						for vault_id in pool_info.assets_vault_ids {
